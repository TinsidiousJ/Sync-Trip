...
async function searchDestinationsFromGeoapify(text, limit = 8) {
  if (!GEOAPIFY_API_KEY) throw new Error("Missing GEOAPIFY_API_KEY");
  ...
  if (!res.ok) {
    throw new Error(data?.message || "Geoapify destination autocomplete failed");
  }
  ...
  const results = rows
    .map((row) => {
      ...
      if (!allowedTypes.has(resultType)) return null;
      ...
      const formatted = row.formatted || primary;
      const subtitle = country && formatted !== country ? country : "";

      const key = `${formatted}-${row.place_id || ""}`;
...
async function fetchTripadvisorLocationPhotos(locationId) {
  ...
  if (!res.ok) {
    throw new Error(data?.message || `Tripadvisor photos failed for ${locationId}`);
  }
...
function normaliseTripadvisorNearbyHotel(item, detail = null, photos = []) {
  ...
  const explicitPrice =
    ...
    safeNumber(base?.price_level_numeric) ??
    safeNumber(item?.price_level_numeric) ??
    null;
  ...
  const image = extractTripadvisorImage(base, item, photos);
...
async function searchTripadvisorHotels(destination, limit = 12) {
  ...
  const enrichedRows = await Promise.all(
    sliced.map(async (row) => {
      ...
      try {
        ...
        console.error(`Tripadvisor detail lookup failed for ${locationId}:`, err.message);
      }

      try {
        photos = await fetchTripadvisorLocationPhotos(locationId);
      } catch (err) {
        console.error(`Tripadvisor photo lookup failed for ${locationId}:`, err.message);
      }
  ...
  const normalised = enrichedRows
    .map(({ row, detail, photos }) => {
      const hotel = normaliseTripadvisorNearbyHotel(row, detail, photos);

      if (!hotel.image) {
        console.log("No Tripadvisor hotel image found for:", hotel.title, {
          locationId: row?.location_id,
          photosReturned: Array.isArray(photos) ? photos.length : 0,
          hasRowPhoto: Boolean(row?.photo),
          hasDetailPhoto: Boolean(detail?.photo),
        });
      }
  ...
  console.log(
    "Tripadvisor hotel results:",
    normalised.map((hotel) => ({
      title: hotel.title,
      hasImage: Boolean(hotel.image),
      price: hotel.price,
      priceLevelText: hotel.priceLevelText || "",
      rating: hotel.rating,
    }))
  );
...
function normaliseYelpActivity(business = {}) {
  ...
  return {
    ...
    title,
    subtitle,
    price: typeof business.price === "string" ? business.price.length : null,
    currency: "GBP",
...
async function searchYelpActivities(destination, limit = 12) {
  ...
  return detailedBusinesses
    .map(normaliseYelpActivity)
    .filter((item) => item && item.title && item.sourceId);
}
...
app.get("/locations/destinations", async (req, res) => {
  try {
    ...
    const results = await searchDestinationsFromGeoapify(text, 8);
    res.json({ results });
  } catch (err) {
